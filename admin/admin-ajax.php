...
function csrf_pass($nonce_name, $extra_args = array(), $json_output = true)
{
	if ( ! csrfguard_check_referer($nonce_name))
	{
		$output_msg = 'Invalid token or session expired. Please revisit this page and try again.';
		...
		if ($json_output)
		{
			exit(json_encode(array_merge(array(	
				...
				'msg' => $output_msg, 
				'html' => pm_alert_error($output_msg, false, true)),
				$extra_args
			)));
		}
		else
...
function access_denied($extra_args = array(), $json_output = true)
{
	$output_msg = 'Sorry, you do not have access to this area.';
	if ($json_output)
	{
		exit(json_encode(array_merge(array(	
			'success' => false, 
			...
			'msg' => $output_msg, 
			'html' => pm_alert_error($output_msg)),
			$extra_args
		)));
	}
...
if ($page == '')
{
	...
	exit(json_encode(array(
		...
		'alert_type' => 'error',
		'msg' => $msg, 
		'html' => pm_alert_error($msg, false, true)
	)));
}
...
switch ($page)
{
		...
		switch ($action)
		{
			case 'checkurl':
				...
				if (mysql_num_rows($result) > 0)
				{
					...
					$msg = 'This URL was already found into your database! <a href="edit-video.php?vid='. $uniq_id. '">Edit</a> video.';
					$msg_color = 'red';
				}
				else
				{
			...
			case 'generate-video-slug':
				...
				if ($_POST['video-title'] != '')
				{
					...
					$text = get_unique_video_slug($text);
					$text = urldecode($text); 
					exit($text);
				}
	...
	case 'manage-categories':
		...
		switch ($action)
		{
			...
			case 'add-series-genre':
				...
				switch ($action)
				{
					...
					case 'add-series-genre':
						...
						$ajax_msg = categories_dropdown($genres_dropdown_options);
						$genres_dropdown_options = array(
							'db_table' => 'pm_genres',
						...
						$create_category_select_html = categories_dropdown($genres_dropdown_options); 
			...
			case 'delete':
				...
				{
					$ajax_msg = 'Invalid token or session expired. Please revisit this page and try again.';
			...
			case 'organize':
				...
				$sql_table = ($_POST['type'] == 'genre') ? 'pm_genres' : $sql_table;
				
				$tree = $_POST['tree'];
				$total_items = pm_count($tree);
				
				$order = array($tree[1]['item_id'] => array(
					'id' => $tree[1]['item_id'],
					'parent_id' => 0,
					'position' => 1
				)); 
				...
				if (pm_count($order) > 0)
				{
					...
					foreach ($order as $category_id => $c)
					{
						...
						{
							$errors[] = 'An error occurred while updating your category: '. mysql_error();
						}
					}
			...
			case 'mark-featured':
				...
				$featured_var = ($category_type == 'video') ? 'featured_categories' : 'featured_genres';

				if ( ! in_array($category_id, $$featured_var))
				{
					// add

					$$featured_var = array_merge(array($category_id), $$featured_var);
				}
				else
				{
					// remove
					foreach ($$featured_var as $k => $id)
					{
						if ($id == $category_id)
						{
				...
				exit(json_encode(array(	
					'success' => true, 
					'alert_type' => 'success',
					'msg' => ($category_type == 'video') 
						? 'Videos from this category will now appear on your homepage'
						: 'Episodes from this genre will now appear on the series page',
					'html' => '<i class="mi-search"></i>'
				)));
	...
	case 'layout-settings': // theme-settings.php
	...
	case 'utilities':
		...
		switch ($action)
		{
			...
			case 'set-ui-prefs':
				...
				set_admin_ui_prefs($_POST['ui_pref_name'], $_POST['ui_pref_value']);
	...
	case 'readlog':
		...
		switch ($action)
		{
			case 'mark-all-read':
				...
				{
					exit(json_encode(array('success' => false, 'msg' => pm_alert_error('Invalid token or session expired. Please revisit this page and try again.'))));
				}
				...
				switch ($query_params['data_source'])
				{
					...
					case 'youtube-channel':
						...
						else //if ($sub['sub_type'] == 'user' || $sub['sub_type'] == 'user-favorites' || $sub['sub_type'] == 'user-playlist')
						{
							...
							if (pm_count($api_data['results']) > 0)
	...
	case 'import':
		...
		switch ($action)
		{
			...
			case 'search':
				...
				switch ($data_source)
				{
					...
					case 'csv':
						...
						while ($row = mysql_fetch_assoc($result))
						{
							...
							if (pm_count($geo_restriction) == 0 || ! is_array($geo_restriction)) // allowed everywhere
							{
							...
							switch ($row['source_id'])
							{
								...
								case $sources['youtube']['source_id']:
									...
									$row['embed_url'] = '//www.youtube.com/embed/'. $row['yt_id'] .'?&autoplay=1&v='. $row['yt_id'] .'&version=3';
				...
				// begin formatting
				$alt = 0;
				$total_results = pm_count($api_data['results']);
				$counter = 0;
				$duplicates = 0;
				$total_search_results = $api_data['meta']['total_results'];
				...
				if ( ! $sub_id && ! doing_cron())
				{
					...
					if (pm_count($overwrite_category) > 0)
					{
						$query_params['oc'] = 1;
...
//					}
				...
				switch ($import_action)
				{
							...
							switch ($import_action)
							{
								...
								case 'playlists': // @since v2.5
									...
									$sub_params = array(
														...
														'autofilling' => $autofilling,
														'autodata' => $autodata,
														'oc' => (pm_count($overwrite_category)) ? 1 : 0,
														'utc' => (pm_count($overwrite_category)) ? implode(',', $overwrite_category) : ''
													);
								break;
								// @since v2.5
								/*
					...
					case 'list-playlists': // show user playlists
						...
						if ($playlists['meta']['total_results'] > 0)
						{
							...
							?>
							<div class="import-playlists col-md-12">
								<div class="card">
									<div class="card-header header-elements-inline"><h5>Playlists</h5></div>
									<div class="card-body">
										<ul class="media-list media-list-linked">
											<?php foreach ($playlists['results'] as $i => $item) : 
												...
												<li class="">
												<?php endif; ?>
												<a href="import-user.php?action=playlists&username=<?php echo $username; ?>&results=<?php echo $import_results; ?>&playlistid=<?php echo $item['id']; ?>&title=<?php echo urlencode($item['title']); ?>&autofilling=<?php echo $autofilling; ?>&autodata=<?php echo $autodata; ?>&oc=1&utc=<?php echo implode(',', $overwrite_category) .'&data_source='. $data_source .'&sub_id='. $subscription_id; ?>" class="import-user-playlist-item media" data-playlist-id="<?php echo $item['id']; ?>" data-playlist-title="<?php echo htmlspecialchars($item['title']); ?>">
													<div class="mr3">
														<img src="<?php echo $item['playlist_thumb_url']; ?>" alt="" width="120" height="67" class="rounded">
													</div>
							...
							exit(json_encode(array(	'success' => true,
													...
													'total_results' => pm_count($playlists['results']),
			...
			case 'import':
				...
				$total_videos = pm_count($_POST['video_ids']);
				$imported_total = 0;
			...
			case 'process-queue':
				...
				switch ($ajax_state)
				{
					...
					case 'finished':
						...
						if ( $items_processed > 0)
						{ 
							$ajax_msg .= '<div class="pm-file-action-next-step mb-2">';
							$ajax_msg .= '<a href="'. _URL .'/'. _ADMIN_FOLDER .'/import-csv.php?step=3&file-id='. $csv_file['file_id'] .'" class="btn btn-success">Continue with Import <i class="mi-navigate-next"></i></a>';
							$ajax_msg .= '</div>';
						}
						else 
						{
							$ajax_msg .= '<div class="pm-file-action-next-step mb-2">';
							$ajax_msg .= '<a href="'. _URL .'/'. _ADMIN_FOLDER .'/import-csv.php" class="btn btn-primary">Upload another CSV</a>';
							$ajax_msg .= '</div>';
						}
	...
	case 'manage-comments':
		...
		switch ($action)
		{
			...
			case 'delete':
				...
				if( ! $logged_in || ! is_admin() || (is_moderator() && mod_cannot('manage_comments')))
				{
					$ajax_msg = 'Sorry, you do not have access to this area.';
                    exit(json_encode(array( 
                    	'success' => false, 
				...
				csrf_pass('_admin_episodes_listcontrols');
				...
				if ( ! $result = mysql_query($sql))
				{
					$ajax_msg = 'Failed to update database: <code>'. mysql_error() .'</code>.';
					exit(json_encode(array( 
                    	'success' => false, 
                        'alert_type' => 'error',
                        'msg' => $ajax_msg, 
				...
				$nonce = ($_REQUEST['_pmnonce'] != '') ? csrfguard_raw($_REQUEST['_pmnonce']) : array();

				$ajax_msg = 'Comments successfully removed.';
				exit(json_encode(array( 
                    'success' => true, 
                    'alert_type' => 'success',
                    '_pmnonce' => $nonce['_pmnonce'],
...
//			break;
			...
			case 'view-log':
				...
				<table cellpadding="0" cellspacing="0" width="100%" class="table table-xs table-striped tablesorter">
					<thead>
						<tr>
							<th style="width:170px; min-width: 170px;">Log Date</th>
							<th>Details</th>
						</tr>
					</thead>
					<tbody>
					...
					<tr class="font-weight-semibold">
						<td><small><?php echo date('M d, Y h:i:s A', $job['data']['time_last_run']); ?></small></td>
						<td>
					...
					<?php endif; ?>
					<?php if (is_array($log) && pm_count($log) > 0) :
						foreach ($log as $k => $log_data) : ?>
						<tr>	
							<td><small><?php echo date('M d, Y h:i:s A', $log_data['time']); ?></small></td>
						...
						<?php if ( ! (in_array($job['type'], array('vscheck', 'sitemap', 'video-sitemap')) && $job['data']['time_started'] > 0)): ?>
							<tr>
								<td colspan="2" align="center" class="text-center">Nothing logged yet.</td>
							</tr>
						<?php endif; ?>
					<?php endif; ?>
					</tbody>
	...
	case 'upload':
		...
		switch ($action)
		{
			case 'upload-image': // moved from upload-image.php @since v2.7
				
				header('Content-Type: text/plain; charset=utf-8');
				
				$whitelist	  = array('.jpg', '.gif', '.png', '.jpeg','.mp4');
				$allowed_type = array('mp4', 'image/gif', 'image/jpg', 'image/jpeg', 'image/pjpeg');
				$upload_errors = array(
							...
							if ($file['error'] == 0)
							{
								if ($file['size'] > 0)
								{
									...
									if ($_POST['upload-type'] != 'logo' 
										&& $_POST['upload-type'] != 'video-thumb' 
										&& $_POST['upload-type'] != 'category-image' 
										&& $_POST['upload-type'] != 'article-thumb' 
										&& $_POST['upload-type'] != 'series-image'
									) {
										// WYSIWYG editor 
										...
										if ($move !== false)
										{
											...
											{
												$html="<video controls='controls' width='300' height='150' vspace='' hspace='' border='1' alt='' >
<source src='$file_url' type='video/mp4' /></video>";
									...
									if ($_POST['upload-type'] == 'video-thumb')
									{
										...
										else
										{
											...
											$html = '<img id="show-thumb" class="show-thumb-temp img-fluid" src="'. $file_url .'?cachebuster='. time() .'" width="" height=""';
											$html .= ' vspace="" hspace="" border="0" alt="" />';
									...
									if ($_POST['upload-type'] == 'category-image')
									{
										...
										else
										{
											...
											$html = '<img id="show-cat-cover" class="show-thumb-temp img-responsive" src="'. $file_url .'?cachebuster='. time() .'" width="" height=""';
											$html .= ' vspace="" hspace="" border="0" alt="" />';
									...
									if ($_POST['upload-type'] == 'series-image')
									{
										$img = new resize_img();
										$img->sizelimit_x = 185;
										$img->sizelimit_y = 278;
										...
										$random_str = substr(md5($series_id . time()), 0, 11);
										$file_url = _THUMBS_DIR . $random_str . '.jpg';
										...
										{
											$img->save_resizedimage(_THUMBS_DIR_PATH, $random_str);
											$html = '<img id="show-series-image" class="show-opt-link" data-opt-target-id="show-opt-thumb" src="'. $file_url .'?cachebuster='. time() .'" width="" height=""';
											$html .= ' vspace="" hspace="" border="0" alt="" />';
											$html .= '<input type="hidden" name="image" value="'. $random_str .'.jpg" />';
										}
									...
									if ($_POST['upload-type'] == 'article-thumb')
									{

										$img = new resize_img();
										$img->sizelimit_x = THUMB_W_ARTICLE;
										$img->sizelimit_y = THUMB_H_ARTICLE;
										$img->keep_proportions = true;
										$img->output = 'JPG';
										...
										$new_name = $uniq_id .'_th';
										$file_url = _ARTICLE_ATTACH_DIR . $new_name . '.jpg';
										...
										{
											$img->save_resizedimage(_ARTICLE_ATTACH_DIR_PATH, $new_name);
											
											$html = '<img id="show-thumb" class="show-thumb-temp img-fluid" src="'. $file_url .'?cachebuster='. time() .'" width="" height=""';
											$html .= ' vspace="" hspace="" border="0" alt="" />';
											$html .= '<input type="hidden" name="post_thumb_show" value="'.$new_name.'.jpg" />';

											// @since 2.7.1 attempt to set file permissions 
											$stat = @stat(dirname(_ARTICLE_ATTACH_DIR_PATH . $new_name .'.jpg'));
											$perms = $stat['mode'] & 0000666;
											@chmod(_ARTICLE_ATTACH_DIR_PATH . $new_name .'.jpg', $perms);
			...
			case 'upload-file':  // moved from upload-file.php @since v2.7
				...
				else if ($_POST['upload-type'] == 'video' || $_POST['upload-type'] == 'episode')
				{
				...
				if ($allow == 1)
				{
					if ($_POST['upload-type'] == 'csv')
					{
						...
						$html .= '<div class="pm-file-action"><a href="'. _URL .'/'. _ADMIN_FOLDER .'/import-csv.php?step=2&file-id='. $file_id .'" class="btn btn-success">Continue <i class="mi-navigate-next"></i></a></div>';
					...
					if ($move !== false)
					{
						if ($_POST['upload-type'] == 'subtitle')
						{
							if ($_POST['uniq_id'] != '')
							{
								...
								if (is_array($row))
								{
									...
									{
										$html .= '<li id="subtitle-'. $sub['id'] .'">';
										$html .= '<span class="float-right">';
										$html .= '<a href="'. _SUBTITLES_DIR . $sub['filename'] .'" title="Download file" target="_blank"><i class="icon-download"></i></a> ';
										$html .= '<a href="" title="Delete subtitle" data-sub-id="'. $sub['id'] .'" onclick="return delete_subtitle('. $sub['id'] .')" class="text-danger ml-1"><i class="icon-bin"></i></a>';
										$html .= '</span>';
										$html .= '<strong>'. ucfirst($sub['language']) .'</strong>';
										if ($sub['filename'] == $new_name)
										{
											$html .= ' <span class="badge badge-info">updated</span>';
										}
										$html .= '</li>';
								...
								else
								{
									...
									foreach ($subtitles as $k => $sub)
									{
										...
										$html .= '<span class="float-right">';
										$html .= '<a href="'. _SUBTITLES_DIR . $sub['filename'] .'" title="Download file" target="_blank"><i class="icon-download"></i></a>';
										$html .= '<a href="" title="Delete subtitle" data-sub-id="'. $sub['id'] .'" onclick="return delete_subtitle('. $sub['id'] .')" class="text-danger ml-1"><i class="icon-trash"></i></a>';
										$html .= '</span>';
										$html .= '<strong>'. ucfirst($sub['language']) .'</strong>';
										if ($sub['filename'] == $new_name)
										{
											$html .= ' <span class="badge badge-success">uploaded</span>';
										}
										$html .= '</li>';
									}
						...
						if ($_POST['upload-type'] == 'video')
						{
							if ($_POST['uniq_id'] != '')
							{
								...
								if ($result = mysql_query($sql))
								{
									...
									$html = '<span class="float-right">';
									$html .= '<i class="icon-download"></i> <strong><a href="'. _VIDEOS_DIR . $new_name .'" title="Download file">Download</a></strong>';
									$html .= '</span>';
									$html .= '<strong>'. $new_name .'</strong> <span class="badge badge-success">updated</span>';
									...
									exit(json_encode(array(
										...
										'alert_type' => 'success',
										'filename' => $new_name,
										'filepath' => _VIDEOS_DIR_PATH . $new_name,
										'fileurl' => _VIDEOS_DIR . $new_name,
										'msg' => '',
										'html' => $html
									)));
								}
						...
						else if ($_POST['upload-type'] == 'episode')
						{
							$ms = episode_add_media_source($_POST['episode_id'], 'file', $new_name);
						...
						else 
						{
							...
							if (is_array($result))
							{
								$fp = @fopen('./temp/upload-file.tmp', "a");
				...
				if ($error_msg != '')
				{
					if ($_POST['upload-type'] == 'video' || $_POST['upload-type'] == 'episode')
					{
						...
						if (strpos($error_msg, "0 bytes") !== false)
						{
							...
							$log_msg .= '<br />Meanwhile you can upload the video(s) with an FTP client into the <strong>/uploads/videos/</strong> folder and add them to your site using the "<a href="add-video.php">Add Video from URL</a>" page.';
						}
	...
	case 'series':
		...
		if ($action == 'generate-series-slug')
		{
			if ($_POST['title'] != '')
			{
				$text = trim($_POST['title']);
				$text = sanitize_title($text);
				$text = get_unique_series_slug($text);
				$text = urldecode($text); 
				exit($text);
			}
		...
		csrf_pass('_admin_series_listcontrols');

		switch ($action)
		{
			case 'delete':
				
				$result = delete_series($_POST['series_id']);
				
				$nonce = csrfguard_raw('_admin_series_listcontrols');

				exit(json_encode(array(
					'success' => ($result['type'] == 'error') ? false : true, 
					'alert_type' => ($result['type'] == 'error') ? 'error' : 'success',
					'_pmnonce' => $nonce['_pmnonce'],
					'_pmnonce_t' => $nonce['_pmnonce_t'],
					'msg' => $result['msg'], 
					'html' => ($result['type'] == 'error')
						? pm_alert_error($result['msg'], false, true)
						: pm_alert_success($result['msg'], false, true)
				)));
	...
	case 'episodes':
		...
		if ( ! $logged_in || ! is_admin() || (is_moderator() && mod_cannot('manage_videos')))
		{
			access_denied();
		}
		...
		switch ($action)
		{
			case 'edit':
				...
				if ($_POST['episode_id'] == 0)
				{
					...
					{
						$updated_episode_data = get_episode($result['episode_id']);
					}
				}
				else
				{
					$result = update_episode($_POST['episode_id'], $_POST);
					$updated_episode_data = get_episode($_POST['episode_id']);
				}

				exit(json_encode(array(
					'success' => ($result['type'] == 'error') ? false : true, 
					'alert_type' => ($result['type'] == 'error') ? 'error' : 'success',
					'episode_data' => $updated_episode_data,
					// '_pmnonce' => $nonce['_pmnonce'],
					// '_pmnonce_t' => $nonce['_pmnonce_t'],
					'msg' => $result['msg'], 
					'html' => ($result['type'] == 'error')
			...
			case 'delete':
				...
				csrf_pass('_admin_episodes_listcontrols');
				$nonce = csrfguard_raw('_admin_episodes_listcontrols');

				$result = delete_episode($_POST['episode_id']);
			...
			case 'delete-media-source-file':

				csrf_pass('_admin_edit_episode'. $_REQUEST['episode_id']);
				$nonce = csrfguard_raw('_admin_edit_episode'. $_REQUEST['episode_id']);
				$result = true; 
				
				if ( ! empty($_POST['ms_id']) || ! empty($_POST['ms_type']) || ! empty($_POST['ms_data']))
					$result = episode_remove_media_source($_POST['episode_id'], $_POST['ms_id'], $_POST['ms_type'], $_POST['ms_data']);
				
				exit(json_encode(array(
					'success' => $result, 
					'alert_type' => ( ! $result) ? 'error' : 'success',
					'_pmnonce' => $nonce['_pmnonce'],
					'_pmnonce_t' => $nonce['_pmnonce_t'],
					'msg' => ( ! $result) ? 'MySQL Error: '. mysql_error() : 'Success',
					'html' => ( ! $result)
						? pm_alert_error('MySQL Error: '. mysql_error(), false, true)
						: pm_alert_success($result['msg'], false, true)
				)));
